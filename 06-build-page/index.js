...
fs.readFile(path.join(__dirname, 'template.html'), 'utf-8', (err, dataTemplate) => {
    ...
    let regexpTemplateTag = /\{\{+[\w]+}}/g;
    let tagTemplates = dataTemplate.match(regexpTemplateTag);
    let changedTag = dataTemplate;
    for (let tagTemplate of tagTemplates) {
        fs.readFile(path.join(__dirname, 'components',
         `${tagTemplate.split('}}').shift().split('{{').pop()}.html`), 
         'utf-8', (err, dataComponents) => {
            changedTag = changedTag.replace(tagTemplate, dataComponents)
